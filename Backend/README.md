...
## Future Improvements
- **Role-Based Access Control (RBAC)**: Ensure only admin users can access POST/PATCH/DELETE endpoints on products.
- **Pagination & Filters**: Support search, filters, and pagination for product listings (`GET /user/product`).
- **Comprehensive Unit Testing**: Write Jest integration and unit tests for the middleware and routers.
- **HTTPS & Secure Cookies**: Configure cookie flags (`secure: true`, `sameSite: "strict"`) for secure sessions in production environments.
